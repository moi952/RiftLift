...
bool InputManager::LoadActionManifest()
{
	...
	const char* devManifest = getenv("RIFTLIFT_ACTION_MANIFEST");
	if (devManifest)
	{
	...
	LSTATUS status = RegGetValueA(HKEY_LOCAL_MACHINE, "Software\\RiftLift", "", RRF_RT_REG_SZ | RRF_SUBKEY_WOW6432KEY, NULL, NULL, &pathSize);
	if (status == ERROR_SUCCESS)
	{
		pathVec.resize(pathSize);
		status = RegGetValueA(HKEY_LOCAL_MACHINE, "Software\\RiftLift", "", RRF_RT_REG_SZ | RRF_SUBKEY_WOW6432KEY, NULL, pathVec.data(), &pathSize);
...
void InputManager::OculusTouch::HapticsThread(OculusTouch* device)
{
	...
	while (device->m_bHapticsRunning)
	{
		...
		if (sample > 0.0f)
		{
			// Avoid continuously entering the native OpenVR client from both
			// controller threads when there is no haptic work. Proton's vrclient
			// bridge can serialize device-role queries internally, and two idle
			// polling threads otherwise race during controller initialization.
